/***********************************************************************************************************************
	...
	macros/macrolib/numberbox-textbox.js
...
Macro.add(['numberbox', 'textbox'], {
	...
	handler() {
		...
		if (autofocus) {
			...
			// Set up a single-use `:passagedisplay` handler to autofocus the element.
			jQuery(document).one(':passagedisplay', () => setTimeout(() => el.focus(), MIN_DOM_ACTION_DELAY));
		}
	}
});
