...
(() => {
	...
	*******************************************************************************/
	...
	function _processAttributeDirectives(el) {
		// NOTE: The `.attributes` property yields a live collection, so we must make
		// a non-live copy of it as we will be adding and removing members of said
		// collection if any directives are found.
		Array.from(el.attributes).forEach(({ name, value }) => {
			...
			if (evalShorthand || name.startsWith('sc-eval:')) {
				...
				try {
					// NOTE: Most browsers (ca. Nov 2017) have broken `setAttribute()` method
					// implementations that throw on attribute names that start with, or contain,
					// various symbols that are completely valid per the specification.  Thus,
					// this code could fail if the user chooses attribute names that, after
					// removing the directive prefix, are unpalatable to `setAttribute()`.
	...
	*******************************************************************************/
				...
				try {
					...
					if (macro) {
						...
						if (typeof macro.handler === 'function') {
							...
							try {
								// QUESTION: Keep passing in the context and args?
								macro.handler.call(this.context, this.context, ...this.context.args);
							}
							finally {
					...
					switch (item.type) {
						...
						case Item.SquareBracket: {
							...
							if (markup.isLink) {
								...
								arg = { isLink : true };
								arg.count    = hasOwn(markup, 'text') ? 2 : 1;
								arg.link     = Wikifier.helpers.evalPassageId(markup.link);
								arg.text     = hasOwn(markup, 'text') ? Wikifier.helpers.evalText(markup.text) : arg.link;
								arg.external = !markup.forceInternal && isExternalLink(arg.link);
								arg.setFn    = hasOwn(markup, 'setter')
	...
	Wikifier.Parser.add({
		...
		handler(w) {
			...
			const text  = hasOwn(markup, 'text') ? Wikifier.helpers.evalText(markup.text) : link;
			const setFn = hasOwn(markup, 'setter')
				? Wikifier.helpers.createShadowSetterCallback(Scripting.parse(markup.setter))
				: null;
			...
			if (markup.forceInternal || !isExternalLink(link)) {
				createLink(output, link, text, setFn);
			}
			else {
				createExternalLink(output, link, text);
			}
		}
	});
		...
		handler(w) {
			...
			if (Config.debug) {
				debugView = new DebugView(
					w.output,
					'image-markup',
					hasOwn(markup, 'link') ? '[img[][link]]' : '[img[]]',
			...
			if (hasOwn(markup, 'link')) {
				...
				if (markup.forceInternal || !isExternalLink(link)) {
					el = createLink(el, link, null, setFn);
				}
				else {
					el = createExternalLink(el, link);
				}
		...
		*/
		...
		handler(w) {
			new Wikifier(
				...
				stringFrom(State.getVar(w.matchText))
			);
		}
	});
	...
	Wikifier.Parser.add({
		...
		handler(w) {
			jQuery(document.createDocumentFragment())
				.append(w.options.nobr ? w.matchText : document.createElement('br'))
				.appendTo(w.output);
		}
	});
			...
			if (match && match.index === w.matchStart) {
				...
				if (this.hasImageMarkup.test(css)) {
					...
					css = css.replace(this.imageMarkup, wikiImage => {
						...
						if (hasOwn(markup, 'error') || markup.pos < wikiImage.length) {
							return wikiImage;
						}
		...
		handler(w) {
			...
			if (match && match.index === w.matchStart) {
				...
				$frag.find('a[data-passage],image[data-passage]').each((_, el) => {
					...
					try {
						_processAttributeDirectives(el);
					}
					catch (ex) {
						return appendError(
							w.output,
				...
				if (tagName === 'image') {
					if (passage.slice(0, 5) !== 'data:' && Story.has(passage)) {
						...
						if (passage.tags.includes('Twine.image')) {
							...
							el.setAttribute('href', passage.source.trim());
						}
					}
				}
		...
		handler(w) {
			...
			if (tagName) {
				...
				if (isVoid || terminatorMatch) {
					...
					// NOTE: The use of `cloneNode(true)` here for `<track>` elements is
					// necessary to workaround a poorly understood rehoming issue.
				...
				if (this.mediaTags.includes(tagName)) {
					if (passage.slice(0, 5) !== 'data:' && Story.has(passage)) {
						...
						if (passage.tags.includes(twineTag)) {
							el[parentName === 'picture' ? 'srcset' : 'src'] = passage.source.trim();
						}
					}
				}
