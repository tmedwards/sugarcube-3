...
/*
	Setting API static object.
*/
const Setting = (() => {
	// Setting control types object (pseudo-enumeration).
	const Types = mappingFrom({
		Header : 0,
		Toggle : 1,
		List   : 2,
		Range  : 3
	...
	*******************************************************************************/
	...
	// The settings data storage key.
	const STORAGE_KEY = 'settings';

	function settingsInit() {
		if (BUILD_DEBUG) { console.log('[Setting/settingsInit()]'); }
	...
	function settingsSave() {
		...
		return Db.storage.set(STORAGE_KEY, savedSettings);
	}

	function settingsLoad() {
		const defaultSettings = settingsCreate();
		const loadedSettings  = Db.storage.get(STORAGE_KEY) || settingsCreate();
		...
		switch (type) {
			...
			case Types.Range:
				...
				if (!def.hasOwnProperty('step')) {
					...
					const stepValidate = function (value) {
						if (fracDigits > 0) {
...
/*
	Module Exports.
*/
export { Setting as default, settings };
