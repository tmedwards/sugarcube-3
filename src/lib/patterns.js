...
const Patterns = (() => {
	...
	*/
	...
	// Macro name pattern.
	const macroName = `(?:${identifierStart.replace('\\$', '').replace('_', '')}${identifierPart.replace('[\\$', '[\\x2D')}*|[\\x2D=])`;
	...
	*/
	const cssIdentifierStart = '(?:\\x2D?(?:[A-Z\\\\_a-z\\x80-\\uD7FF\\uE000-\\uFFFF]|[\\uD800-\\uD803][\\uDC00-\\uDFFF]|[\\uD800-\\uDBFF](?![\\uDC00-\\uDFFF])|(?:[^\\uD800-\\uDBFF]|^)[\\uDC00-\\uDFFF])|\\x2D\\x2D)';
	const cssIdentifierPart  = '(?:[\\x2D0-9A-Z\\\\_a-z\\x80-\\uD7FF\\uE000-\\uFFFF]|[\\uD800-\\uD803][\\uDC00-\\uDFFF]|[\\uD800-\\uDBFF](?![\\uDC00-\\uDFFF])|(?:[^\\uD800-\\uDBFF]|^)[\\uDC00-\\uDFFF])';
	const cssIdentifier      = `${cssIdentifierStart}${cssIdentifierPart}*`;
	const cssIdOrClassSigil  = '[#.]';
	// NOTE: Yes, `cssIdOrClass` uses `cssIdentifierPart` rather than `cssIdentifier`.
	const cssIdOrClass       = `${cssIdOrClassSigil}${cssIdentifierPart}+`;
	...
	// Inline CSS pattern.
	const inlineCss = (() => {
		const cssStyle  = `${spaceNoTerminator}*(${cssIdentifier})${spaceNoTerminator}*:([^;\\|\\n]+);`;
		const idOrClass = `${spaceNoTerminator}*((?:${cssIdOrClass}${spaceNoTerminator}*)+);`;
	...
	*******************************************************************************/
	...
	return Object.preventExtensions(Object.create(null, {
		...
		notSpace           : { value : notSpace },
		anyChar            : { value : anyChar },
		identifierStart    : { value : identifierStart },
		identifierPart     : { value : identifierPart },
		identifier         : { value : identifier },
		variableSigil      : { value : variableSigil },
		...
		cssIdOrClassSigil  : { value : cssIdOrClassSigil },
		cssIdOrClass       : { value : cssIdOrClass },
		cssImage           : { value : cssImage },
