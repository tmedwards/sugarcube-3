...
const SimpleAudio = (() => {
	...
	*******************************************************************************/
	...
	class AudioList {
		...
		_next() {
			...
			//
			// QUESTION: Should we reapply the `ended` event handler too?
	...
	*/
	...
	function trackDelete(id) {
		...
		// QUESTION: Should this also remove references to the track from groups and playlists?
	...
	*/
	function runnerGet(/* selector */) {
		...
		try {
			...
			const renderIds = function renderIds(idObj) {
				const id = idObj.id;
				let ids;
